...
(function bootReportPrint() {
    ...
    function buildDocument() {
        ...
        rows.forEach(row => {
            ...
            groups[number].push({date:cells[0]?.textContent?.trim()||"-",subject:cells[2]?.textContent?.trim()||"-",material:cells[3]?.textContent?.trim()||"-",type:cells[5]?.textContent?.trim()||"-"});
        });
        ...
        wrap.innerHTML = `<div class="report-sheet"><header class="report-print-header"><div class="school">SD ISLAM DARUL MU'MININ</div><h1>LAPORAN JADWAL SUMATIF</h1><div>SEMESTER ${esc(semester.toUpperCase())}</div><div>TAHUN PELAJARAN ${esc(year)}</div><div class="class-line">KELAS ${esc(className)}</div></header><table class="formal-report"><thead><tr><th>Sumatif</th><th>Tanggal</th><th>Mata Pelajaran</th><th>Materi</th><th>Jenis Tes</th></tr></thead>${bodies}</table><div class="sts-sas"><div><b>STS :</b><span></span></div><div><b>SAS :</b><span></span></div></div><div class="signatures"><div>Mengetahui,<br>Kepala Sekolah<br><br><br>(________________________)</div><div>Tangerang, __________________ 20____<br>Wali Kelas ${esc(className)}<br><br><br>(________________________)</div></div><div class="footer">Berilmu&nbsp;&nbsp;•&nbsp;&nbsp;Berakhlak&nbsp;&nbsp;•&nbsp;&nbsp;Berprestasi</div></div>`;
        return wrap;
    }

    function installStyles() {
        document.getElementById("reportPrintStyles")?.remove();
        const style = document.createElement("style");
        style.id = "reportPrintStyles";
        style.textContent = `
#reportPrintDocument{display:none!important}
@media print{
 @page{size:215mm 330mm;margin:0!important}
 html,body{margin:0!important;padding:0!important;background:#fff!important}
 body>*{display:none!important}
 body>#reportPrintDocument{display:block!important;visibility:visible!important;position:static!important;width:215mm!important;height:330mm!important}
 body>#reportPrintDocument *{visibility:visible!important}
 .report-sheet{box-sizing:border-box;width:215mm;height:330mm;overflow:hidden;position:relative;padding:7mm 8mm 5mm;background:#fff;color:#172b4d;font-family:Arial,Helvetica,sans-serif}
 ...
 .signatures{position:absolute;left:8mm;right:8mm;bottom:13mm;display:grid;grid-template-columns:1fr 1fr;text-align:center;font-size:7pt;line-height:1.3}.footer{position:absolute;left:0;right:0;bottom:5mm;text-align:center;font-size:5.5pt;font-style:italic;font-weight:700}
    ...
    window.printSumatifReport = printReport;
